...
export const api = {
  ...
  getSimpleLoginStatus: () => request<{ configured: boolean; key_preview?: string }>("/settings/simplelogin"),
  saveSimpleLoginKey: (apiKey: string) => request("/settings/simplelogin", { method: "POST", body: JSON.stringify({ api_key: apiKey }) }),
  deleteSimpleLoginKey: () => request("/settings/simplelogin", { method: "DELETE" }),
  ...
  createRequestFromExposure: (id: number) =>
